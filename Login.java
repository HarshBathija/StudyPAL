...
public class Login {
	...
	private void checkvalidity() throws IOException
	{
		...
		String u,p,l;
		while((l=br.readLine())!=null)                //Till end of file is reached...
		{
			u=l.substring(0,l.indexOf('*'));
			p=l.substring(l.indexOf('*')+1,l.indexOf('~'));
			if((u.compareTo(username)==0)&&(p.compareTo(password)==0))  //U and P are picked up from file.																								
			{                                                           //and changed with every iteration. 
				flag=1;
				break;
			}
